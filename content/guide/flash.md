...
description = "Flash messages are one-shot notices that survive a redirect: set one before a 303 See Other (the \"post-redirect-get\" pattern) and it is shown exactly once on the next page, then cleared. They are the idiomatic way to confirm a successful form submission (\"Post created\") or report a failure (\"Invalid credentials\")."
order = 690
+++
