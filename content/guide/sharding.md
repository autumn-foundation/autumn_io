...
description = "A single Postgres primary — even with read replicas — eventually becomes the write bottleneck. Autumn's sharding story routes tenant data across multiple independent Postgres databases while keeping the application code shard-unaware: routing is declared in autumn.toml, and handlers use the ShardedDb / Shards extractors from the prelude."
order = 890
+++
