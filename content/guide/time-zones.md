+++
title = "Per-user time zones"
description = "Autumn ships a TimeZone extractor that resolves the requesting user's IANA time zone and makes it available in handlers as a first-class parameter. Pair it with the Clock extractor for fully deterministic, test-injectable date/time rendering — no globals, no Utc::now() calls in templates."
order = 850
+++
