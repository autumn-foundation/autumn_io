...
description = "Autumn's built-in presence primitive tells you which users are currently subscribed to a given topic across all replicas. It is the Rust equivalent of Phoenix Presence — per-topic membership, automatic join/leave broadcasting, TTL-based lease eviction — exposed as a single request extractor with no extra client-side dependencies."
order = 510
+++
