+++
title = "Plugin Metrics Sources"
description = "Autumn's /actuator/prometheus and /actuator/metrics endpoints expose the framework's built-in autumn_http_* metric families. The MetricsSource trait lets any in-process subsystem — a background-worker plugin, a workflow engine, an application-level queue — contribute additional families to the same scrape endpoint, with no second port or exporter."
order = 610
+++
