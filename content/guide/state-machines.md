...
description = "Autumn's #[state_machine] field attribute lets you declare valid status transitions directly in your model struct. The macro generates a compile-time transition table, a can_transition_{field}_to predicate, and a transition_{field}_to method that enforces the graph at runtime — no hand-written match blocks or scattered if old_status == \"draft\" { ... } checks spread across route handlers."
order = 400
+++
