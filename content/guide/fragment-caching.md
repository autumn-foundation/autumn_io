+++
title = "Maud fragment caching"
description = "Autumn can cache a rendered Maud fragment keyed by a record's identity and version, so unchanged rows are served from cache and re-render automatically the moment the record is written — without you hand-rolling cache keys or invalidation."
order = 780
+++
