...
description = "When an Autumn handler panics or returns a 5xx, the failure should go somewhere you'll actually see it — Sentry, Honeycomb, Slack, a custom sink — not just scroll past in the logs. Autumn's error-reporting pipeline is the \"configure once, errors go somewhere\" seam, modeled on Rails' `Rails.error.report`."
order = 620
+++
