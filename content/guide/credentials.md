...
description = "Autumn ships a built-in encrypted credentials store that lets you commit secrets to your repository safely. Secrets are encrypted at rest with AES-256-GCM; the only thing that must stay off disk (or be injected via an environment variable) is the master key."
order = 470
+++
