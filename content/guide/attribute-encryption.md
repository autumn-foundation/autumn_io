+++
title = "Attribute Encryption (at-rest column encryption)"
description = "Autumn can encrypt individual #[model] columns at rest. A field marked #[encrypted] is stored as opaque ciphertext in the database but behaves like a normal String in your Rust code: repo.find(id) returns plaintext, and repo.update(..) / inserts accept plaintext. No per-call-site changes, no hand-rolled AEAD wrappers."
order = 440
+++
