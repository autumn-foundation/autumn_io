...
description = "Idempotency keys let clients safely retry mutating HTTP requests (POST, PUT, PATCH, DELETE) without causing duplicate side-effects. The client picks a unique key, attaches it as an Idempotency-Key request header, and replays the identical request if it suspects the first attempt was lost in transit. Autumn intercepts subsequent requests with the same key and replays the cached response instead of re-executing the handler."
order = 490
+++
