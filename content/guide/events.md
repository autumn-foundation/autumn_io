+++
title = "Events & Listeners (`#[event]` / `#[listener]`)"
description = "Autumn provides a typed domain event bus so you can react to a business event with several decoupled side effects. Instead of one fat handler that does everything inline, you publish a typed event and independent listeners react to it — synchronously in-request, or durably on the background job queue."
order = 720
+++
