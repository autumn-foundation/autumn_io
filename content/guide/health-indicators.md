...
description = "Autumn's /actuator/health and /ready endpoints surface the framework's own state (startup, graceful shutdown, DB pool stats). The HealthIndicator trait lets any component — a payment gateway, a feature-flag service, an SMTP relay, a downstream HTTP API — plug its own health check into those same endpoints."
order = 600
+++
